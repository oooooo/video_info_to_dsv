...
將影片、語音、文字資料，分析並轉為「表格資料」檔案保存，表格的欄位可自定義。

情境舉例：

收容所動物很多，每隻動物的資料需一筆筆比對、建檔。
現在可以拍攝一段影片，或一段口頭說明錄音、或既有文字檔，用手邊工具快速產生每隻狗狗的資料後，將素材放入程式的指定資料夾 → 執行程式 → 產生整理好的表格資料，自動化一次產生多個動物檔案。
